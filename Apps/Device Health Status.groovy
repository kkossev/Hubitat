...
String displayTable() {
    ...
    devices.sort { it?.displayName.toLowerCase() }.each { dev ->
        ...
        if (settings?.hideNotBatteryDevices == true && state.devices["$dev.id"].hasBattery == false) {
            ...
            def lastBattery = dev.currentBattery ?: 'n/a'
...
void logWarn(msg) { if (logEnable) { log.warn(msg) } }

void logInfo(msg) { if (txtEnable) { log.info(msg) } }
