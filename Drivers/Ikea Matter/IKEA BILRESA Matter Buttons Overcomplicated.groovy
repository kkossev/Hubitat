...
@Field static final Integer POST_PING_JSON_CHECK_DELAY_SEC        = 2                   // Delay before JSON check after a successful ping
@Field static final Integer EARLY_JSON_CHECK_DELAY_SEC            = 10                  // Delay before the first post-subscribe JSON check
@Field static final Integer EVENT_SERIAL_TOLERANCE                = 20                  // Allowed out-of-order serial gap before rejecting as stale
@Field static final Integer CONSECUTIVE_PING_FAIL_THRESHOLD       = 2                   // Consecutive ping failures before marking offline
@Field static final Integer PING_TIMEOUT_SEC                      = 30                  // Timeout for a health-check ping response
@Field static final Integer AUTO_REINIT_DELAY_SEC                 = 30                  // Delay before automatic re-initialize after ping failures
@Field static final Integer HOLD_SIMULATE_DELAY_MS                = 750                 // Delay before simulating a held event
@Field static final Integer MULTI_PRESS_DEBOUNCE_MS               = 150                 // Debounce delay before firing multi-press completion
@Field static final Integer SINGLE_PRESS_DELAY_MS                 = 400                 // Delay used to confirm a single press
@Field static final Integer INIT_PENDING_TIMEOUT_SEC              = 120                 // Fallback timeout for clearing init-pending state
@Field static final Integer FIRST_LIVENESS_CAPTURE_DELAY_SEC      = 625                 // Backup delay to capture the first liveness entry
@Field static final Integer BATTERY_STALENESS_HOURS               = 12                  // Battery age threshold before forcing a fresh read
@Field static final Long    BATTERY_STALENESS_MS                  = 12L * 3600L * 1000L // Battery age threshold in milliseconds
@Field static final Integer DEBUG_LOG_AUTO_DISABLE_SEC            = 86400               // Delay before debug logging auto-disables
@Field static final Integer SUBSCRIPTION_MAX_INTERVAL_DEFAULT_SEC = 900                 // Default Matter subscription max interval
@Field static final Integer TIMESTAMP_PREFIX_SEARCH_CHARS         = 120                 // Characters searched backward for a log timestamp prefix
@Field static final Integer SUBSCRIPTION_LOG_CONTEXT_CHARS        = 3000                // Context window size around a subscription match
@Field static final Integer SUBSCRIPTION_LOG_NARROW_WINDOW_CHARS  = 200                 // Narrow window size for MaxInterval lookup
@Field static final Long    RESUBSCRIBE_THROTTLE_MS               = 10L * 60L * 1000L   // Cooldown between re-subscribe attempts

metadata {
    definition(name: "IKEA BILRESA Matter Buttons Overcomplicated", namespace: "community", author: "kkossev + ChatGPT + Claude", singleThreaded: true, importUrl: "https://raw.githubusercontent.com/kkossev/hubitat/development/Drivers/Ikea%20Matter/IKEA%20BILRESA%20Matter%20Buttons.groovy") {
...
void parse(Map msg) {
    ...
    if (state.pingStart != null) {
        ...
        if (msg.clusterInt == 0x0028 && msg.attrInt == 0x0000) {
            ...
            if (state.subscriptionCheckPending) {
                ...
                if (enableSubscriptionRecovery == true) { runIn(POST_PING_JSON_CHECK_DELAY_SEC, "checkSubscriptionJsonAfterPing") }
            } else {
    ...
    if (msg.callbackType == "SubscriptionResult") {
        ...
        if (enableSubscriptionRecovery == true) {
            ...
            runIn(EARLY_JSON_CHECK_DELAY_SEC, "checkMatterJsonAfterSubscribe", [overwrite: false])
        }
        clearInitPending()
        return
    }
...
private void handleSwitchEvent(Map msg) {
    ...
    if (serial != null) {
        ...
        if (serial < lastSerial - EVENT_SERIAL_TOLERANCE) {
    ...
    switch (msg.evtId) {
        case 1:     // evt 1 – InitialPress; usually followed by LongPress or ShortRelease/MultiPress*
            ...
            if (isWheelModel() && isWheelEndpoint(buttonNumber)) {
                ...
                runInMillis(HOLD_SIMULATE_DELAY_MS, "holdSimulate")
            }
            break
        ...
        case 3: // 3 – ShortRelease
            ...
            if (isWheelModel() && isWheelEndpoint(buttonNumber)) {
                ...
                unschedule("holdSimulate")
                runInMillis(SINGLE_PRESS_DELAY_MS, "singlePressComplete")
            }
        ...
        case 5:     // evt 5 – MultiPressOngoing; value:[0:previousPosition, 1:totalNumberOfPressesSoFar]
            ...
            if (ongoingCount != null && !isWheelModel()) {
                ...
                runInMillis(MULTI_PRESS_DEBOUNCE_MS, "multiPressComplete")
            }
            break
...
void deviceHealthCheck() {
    ...
    runIn(PING_TIMEOUT_SEC, "pingTimeout")
    // Battery staleness check: if no battery report in the last configured window, request a fresh read
    def lastBat = device.currentState("battery")
    if (lastBat == null || (now() - lastBat.date.time) > BATTERY_STALENESS_MS) {
        logWarn "No battery report in >${BATTERY_STALENESS_HOURS}h — requesting battery attribute read"
        sendHubCommand(new HubAction(matter.readAttributes([matter.attributePath(0x00, 0x002F, 0x000C)]), Protocol.MATTER))
    } else {
        logTrace "Battery report is recent (last: ${lastBat.date})"
    }
}
...
void pingTimeout() {
    ...
    if (state.pingConsecutiveFails >= CONSECUTIVE_PING_FAIL_THRESHOLD) {
        ...
        if (enableAutoReInit != false) {
            logWarn "Auto re-init scheduled in ${AUTO_REINIT_DELAY_SEC} seconds"
            runIn(AUTO_REINIT_DELAY_SEC, "autoReInit")
        }
    }
}
...
void updated() {
    logInfo "updated..."
    if (logEnable) runIn(DEBUG_LOG_AUTO_DISABLE_SEC, "logsOff")  // auto-disable debug logging after 24 hours
...
private void subscribeToPaths() {
    ...
    String cmd = matter.cleanSubscribe(0, SUBSCRIPTION_MAX_INTERVAL_DEFAULT_SEC, paths)         // 900s max: fits both BILRESA dual button (~908s liveness) and scroll wheel (~613s liveness)
    ...
    runIn(INIT_PENDING_TIMEOUT_SEC, "clearInitPending")
...
private String extractMatterLogTimestampTokenBeforeIndex(String text, Integer idx) {
    ...
    Integer prefixFrom = Math.max(0, idx - TIMESTAMP_PREFIX_SEARCH_CHARS)
...
private String formatMatterLogWindowPosition(String text, Long eventTimestampMs, String label = "event") {
    if (!text || eventTimestampMs == null) { return "" }
    Map tsRange = extractMatterLogTimestampRange(text)
    Long firstTimestampMs = parseMatterLogTimestampMillis(tsRange.firstTimestamp as String)
    Long lastTimestampMs  = parseMatterLogTimestampMillis(tsRange.lastTimestamp as String)
    if (firstTimestampMs == null || lastTimestampMs == null) { return "" }

    BigDecimal fromStartSeconds = BigDecimal.valueOf(eventTimestampMs - firstTimestampMs).divide(BigDecimal.valueOf(1000L), 3, BigDecimal.ROUND_HALF_UP)
    BigDecimal beforeEndSeconds = BigDecimal.valueOf(lastTimestampMs - eventTimestampMs).divide(BigDecimal.valueOf(1000L), 3, BigDecimal.ROUND_HALF_UP)
    return " [${label} +${fromStartSeconds}s from window start, ${beforeEndSeconds}s before window end]"
}

private String formatMillisDeltaSeconds(Long millis) {
    if (millis == null) { return "n/a" }
    return BigDecimal.valueOf(millis).divide(BigDecimal.valueOf(1000L), 3, BigDecimal.ROUND_HALF_UP).toPlainString()
}

private void rememberLastGoodMatterLogWindow(String text, Long activityTimestampMs, String activitySource = null) {
    if (!text || activityTimestampMs == null) { return }
    Map tsRange = extractMatterLogTimestampRange(text)
    Long firstTimestampMs = parseMatterLogTimestampMillis(tsRange.firstTimestamp as String)
    Long lastTimestampMs  = parseMatterLogTimestampMillis(tsRange.lastTimestamp as String)
    if (firstTimestampMs == null || lastTimestampMs == null) { return }

    state.lastGoodMatterLogWindowFirstMs = firstTimestampMs
    state.lastGoodMatterLogWindowLastMs = lastTimestampMs
    state.lastGoodMatterLogWindowSource = activitySource ?: "activity"
    state.lastGoodMatterLogOffsetFromStartMs = Math.max(0L, activityTimestampMs - firstTimestampMs)
    state.lastGoodMatterLogBeforeEndMs = Math.max(0L, lastTimestampMs - activityTimestampMs)
}
...
void checkMatterJsonAfterSubscribe() {
    ...
    try {
        httpGet([uri: "http://127.0.0.1:8080/hub/matterLogs/json", timeout: 5]) { resp ->
            ...
            Map    result = analyzeMatterJsonForCurrentSubscription(text)
            String subWindowOffset = formatMatterLogWindowPosition(text, result.subscriptionTimestampMs as Long, "sub")
            String reportWindowOffset = formatMatterLogWindowPosition(text, result.reportTimestampMs as Long, "report")
            logDebug "checkMatterJsonAfterSubscribe(): subscriptionSeen=${result.subscriptionSeen}, livenessSeen=${result.livenessSeen}, peer=${result.peer}, livenessMs=${result.livenessMs} — ${result.reason}${result.subscriptionLogTime ? ' [sub logged ' + result.subscriptionLogTime + ']' : ''}${subWindowOffset}${result.reportLogTime ? ' [report logged ' + result.reportLogTime + ']' : ''}${reportWindowOffset}"
            if (result.subscriptionSeen) {
                ...
                if (result.maxIntervalSec) {
                    ...
                    runIn(FIRST_LIVENESS_CAPTURE_DELAY_SEC, "captureFirstLiveness", [overwrite: false])
                }
                rememberLastGoodMatterLogWindow(text, result.activityTimestampMs as Long, result.activitySource as String)
...
void captureFirstLiveness() {
    ...
    try {
        httpGet([uri: "http://127.0.0.1:8080/hub/matterLogs/json", timeout: 5]) { resp ->
            ...
            String subWindowOffset = formatMatterLogWindowPosition(text, result.subscriptionTimestampMs as Long, "sub")
            String reportWindowOffset = formatMatterLogWindowPosition(text, result.reportTimestampMs as Long, "report")
            String livenessWindowOffset = formatMatterLogWindowPosition(text, result.livenessTimestampMs as Long, "liveness")
            logDebug "captureFirstLiveness(): subscriptionSeen=${result.subscriptionSeen}, livenessSeen=${result.livenessSeen}, livenessMs=${result.livenessMs}${result.subscriptionLogTime ? ' [sub logged ' + result.subscriptionLogTime + ']' : ''}${subWindowOffset}${result.reportLogTime ? ' [report logged ' + result.reportLogTime + ']' : ''}${reportWindowOffset}${result.livenessLogTime ? ' [liveness logged ' + result.livenessLogTime + ']' : ''}${livenessWindowOffset}"
            if (result.livenessSeen && result.livenessMs) {
                Integer maxSec  = inferEffectiveMaxIntervalSec(result.livenessMs as Long)
                Integer prevMax = (state.effectiveMaxIntervalSec ?: SUBSCRIPTION_MAX_INTERVAL_DEFAULT_SEC) as Integer
...
private Map analyzeMatterJsonForCurrentSubscription(String text) {
    ...
    Integer fromIdx = Math.max(0, subIdx - SUBSCRIPTION_LOG_CONTEXT_CHARS)
    Integer toIdx   = Math.min(text.length(), subIdx + SUBSCRIPTION_LOG_CONTEXT_CHARS)
    ...
    Integer narrowFrom   = Math.max(0, subIdx - SUBSCRIPTION_LOG_NARROW_WINDOW_CHARS)
    Integer narrowTo     = Math.min(text.length(), subIdx + SUBSCRIPTION_LOG_NARROW_WINDOW_CHARS)
...
private Integer calculateCheckGrace(Integer maxSec) {
    ...
    if (maxSec <= 900) { return 30 }
    return 45
}

private void scheduleNextExpectedLivenessCheck() {
    if (enableSubscriptionRecovery != true) { return }
    Integer maxSec   = (state.effectiveMaxIntervalSec ?: SUBSCRIPTION_MAX_INTERVAL_DEFAULT_SEC) as Integer
    Integer graceSec = calculateCheckGrace(maxSec)
    //Integer delaySec = maxSec + graceSec
    Integer delaySec = maxSec + graceSec + 120   // experiment : 120 seconds later to increase chance of catching the report in the log before it rolls out of the buffer
...
private void markSubscriptionAlive(String source, Long activityTimestampMs = null, String activitySource = null) {
    ...
    String activityLabel = activitySource ?: (activityTimestampMs != null ? 'log' : 'checkTime')
    state.lastSubscriptionActivity = activityMs
    state.lastSubscriptionActivitySource = activityLabel
    state.lastSubscriptionJsonSeen = now()
    state.subscriptionMissCount    = 0
    logDebug "markSubscriptionAlive(): ALIVE [${source}] id=${state.subscriptionIdHex} peer=${state.matterPeer} activity=${formatMatterLogTimestampMs(activityMs)} source=${activityLabel}"
}
...
private void markSubscriptionMissed(String source) {
    ...
    if (state.lastSubscribeSentAt) {
        Long gracePeriodMs = 2L * ((state.effectiveMaxIntervalSec ?: SUBSCRIPTION_MAX_INTERVAL_DEFAULT_SEC) as Long) * 1000L
    ...
    Long lastGoodActivityMs = state.lastSubscriptionActivity as Long
    Long lastGoodAgeSec = null
    if (lastGoodActivityMs != null) {
        Long elapsedSinceLastGoodMs = (now() - lastGoodActivityMs) as Long
        lastGoodAgeSec = elapsedSinceLastGoodMs > 0L ? ((elapsedSinceLastGoodMs / 1000L) as Long) : 0L
    }
    String lastGoodSource = state.lastSubscriptionActivitySource ?: "n/a"
    Long lastGoodWindowFirstMs = state.lastGoodMatterLogWindowFirstMs as Long
    Long lastGoodWindowLastMs = state.lastGoodMatterLogWindowLastMs as Long
    String lastGoodWindowSource = state.lastGoodMatterLogWindowSource ?: "n/a"
    Long lastGoodOffsetFromStartMs = state.lastGoodMatterLogOffsetFromStartMs as Long
    Long lastGoodBeforeEndMs = state.lastGoodMatterLogBeforeEndMs as Long
    logWarn "markSubscriptionMissed(): liveness window missed [${source}] id=${state.subscriptionIdHex} peer=${state.matterPeer} missCount=${missCount}${lastGoodActivityMs != null ? ' lastGood=' + formatMatterLogTimestampMs(lastGoodActivityMs) + ' source=' + lastGoodSource + ' ageSec=' + lastGoodAgeSec : ' lastGood=n/a'}${lastGoodWindowFirstMs != null && lastGoodWindowLastMs != null ? ' lastGoodWindow=' + formatMatterLogTimestampMs(lastGoodWindowFirstMs) + '..' + formatMatterLogTimestampMs(lastGoodWindowLastMs) + ' windowSource=' + lastGoodWindowSource + ' offsetSec=' + formatMillisDeltaSeconds(lastGoodOffsetFromStartMs) + ' beforeEndSec=' + formatMillisDeltaSeconds(lastGoodBeforeEndMs) : ' lastGoodWindow=n/a'}"
...
private Boolean shouldThrottleResubscribe() {
    ...
    return elapsed < RESUBSCRIBE_THROTTLE_MS   // 10-minute cooldown between re-subscribes
}
