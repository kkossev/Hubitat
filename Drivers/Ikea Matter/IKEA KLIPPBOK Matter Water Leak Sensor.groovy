/*
 * IKEA KLIPPBOK Matter Water Leak Sensor (minimal)
 *
 * Last edited: 2025/12/24 5:26 PM
 */
...
metadata {
    definition(name: "IKEA KLIPPBOK Matter Water Leak Sensor", namespace: "community", author: "kkossev + ChatGPT") {
...
void refresh() {
    ...
    // Water/leak state (assumed cluster 0x0045 attr 0x0000 => boolean)
    paths.add(matter.attributePath(0x01, 0x0045, 0x0000))

    // Battery (as in your previous drivers)
...
private void subscribeToAttributes() {
    ...
    if (txtEnable) log.info "Subscribed to KLIPPBOK: water state + battery"
}
...
def parse(String description) {
    ...
    // Water state: cluster 0x0045 attr 0x0000 (expected boolean 0/1)
    if (ep == 0x01 && clus == 0x0045 && attrId == 0x0000) {
        ...
        if (v != null) {
            ...
            if (txtEnable) log.info "Water sensor is ${wet}"
        }
        return
    }

    if (ep == 0x00 && clus == 0x002F && attrId == 0x000C) {
        Integer raw = safeHexToInt(value)   // typically 0..200
        if (raw != null) {
