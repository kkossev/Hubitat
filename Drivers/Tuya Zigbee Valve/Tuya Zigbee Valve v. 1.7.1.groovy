...
metadata {
    definition(name: 'Tuya Zigbee Valve v1.7.1', namespace: 'kkossev', author: 'Krassimir Kossev', importUrl: 'https://raw.githubusercontent.com/kkossev/Hubitat/development/Drivers/Tuya%20Zigbee%20Valve/Tuya%20Zigbee%20Valve%20v.%201.7.1.groovy', singleThreaded: true ) {
        capability 'Actuator'
        capability 'Valve'
        capability 'Refresh'
    ...
    preferences {
        ...
        if (device) {
            ...
            if (isSASWELL() || isGIEX() || isSonoff() || isFankEver() || isTZE284()) {
                input(name: 'autoOffTimer', type: 'number', title: '<b>Auto off timer (Irrigation Duration)</b> ', description: 'Automatically turn off after how many <b>seconds</b> (most models) or <b>minutes</b> (SWV-ZN: 0ΓÇô719 min; TZE284).<br>Zero value disables the auto-off!', defaultValue: DEFAULT_AUTOOFF_TIMER, required: false)
            }
            if (isSASWELL() || isGIEX()) {
            ...
            try {
                ...
                if (bytes.size() >= 15 && schedStatus != (state.states['znLastScheduleStatus'] as Integer)) {
                    ...
                    if (bytes.size() >= 19) {
                        ...
                        long offset = now() / 1000L - devNow   // wall-clock offset: converts device uptime ΓåÆ Unix seconds
                        state.states['znDeviceEpochOffset'] = offset
                        if (schedStatus == 2) {   // first running ΓåÆ emit scheduled start/end
                        ...
                        } else if (schedStatus == 1) {   // end ΓåÆ actual end time + duration
