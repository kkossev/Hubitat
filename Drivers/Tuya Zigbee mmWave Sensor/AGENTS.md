...
> Working guide for AI agents. Written 2026-07-03 against driver **v4.2.4**. Companion documents:
> - `Tuya_Zigbee_mmWave_Sensor_ORIENTATION.md` — deep architecture walkthrough (written at v4.2.3; §1–§10 still accurate, re-verify line numbers before citing them)
> - `Tuya_Zigbee_mmWave_Sensor_IMPROVEMENT_PLAN.md` — approved low-risk improvement backlog (6 items, none implemented yet as of 4.2.4)
> - `Tuya_Zigbee_mmWave_Sensor_OPTIMIZATION_PLAN.md` — 8-step hot-path performance plan + condensed bug-fix backlog (appendix); none applied yet as of 2026-07-03
...
| `Tuya_Zigbee_mmWave_Sensor_ORIENTATION.md` | Architecture deep-dive (v4.2.3 snapshot) | Update if architecture changes |
| `Tuya_Zigbee_mmWave_Sensor_IMPROVEMENT_PLAN.md` | Risk-ranked backlog | Update when items land |
...
1. **Edit `Tuya_Zigbee_mmWave_Sensor.groovy` (and/or the library files), never the `_lib_included` bundle.** The bundle is generated, and only when the maintainer explicitly asks for it — never as a finishing step after a code change. **Never assemble it by hand.** Pasting library bodies after the driver source leaves the source’s `#include` lines in place on top of the inlined libraries, so every library is included twice and the driver will not compile. The Hubitat hub does the assembly: push the driver and all five libraries it includes to a hub, verify each read-back matches the repo (the hub expands from *its own* copies, so a stale library is silently baked in), then `GET /driver/downloadFull/<driverId>` — that endpoint returns the expanded source with the `#include` lines stripped and `// library marker kkossev.<lib>, line N` suffixes added. Save the result with **CRLF** endings; the repo is `core.autocrlf=true` while the endpoint returns LF.
...
- **Cold-start fingerprint gap** — after hub reboot/first install the static caches are empty when `metadata{}` evaluates, so zero fingerprints are declared and newly-pairing devices won't auto-match the driver until profiles load *and* the driver is re-saved. Acknowledged, unfixed (IMPROVEMENT_PLAN item 1).
...
- **Line numbers in ORIENTATION.md** are standalone-library-relative, not bundle-relative, and predate 4.2.4 — re-verify before citing.

## v4.2.4 delta (what changed after ORIENTATION.md was written)
