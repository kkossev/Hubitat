    ...
    dynamicPage(pageProperties) {
        ...
        section(){
            ...
            if(method == "auto")
            {
                ...
                input "refTemp", "double", title: refTempTitle, required: false, range: "0..38", submitOnChange:true
                input "desiredVar", "double", title: "Set a desired variation: by how much do you want inside temperature to differ from outside temp when outside temp is $reF?", required:false, range: "0..38", submitOnChange:true
...
def updateValues(evtVal){
    double outside = outsideTemp.currentValue("temperature").toDouble()
    double outsideThreshold = getOutsideThershold()
    def needSituation = outside < outsideThreshold ? "heat" : "cool"
    double newRefTemp = outside
    double newDesiredVar = Math.abs(evtVal.toDouble() - outside)
...
"""          
    ...
    double absDifference = Math.abs(atomicState.lastSetPoint.toDouble() - evtVal.toDouble())
...
def virtualThermostat(need){
    ...
    {
        def outsideTemperature = outsideTemp?.currentValue("temperature").toDouble() // only needed if electric heater here
        double lowTemperature = lowtemp ? lowtemp : 4
        boolean heaterAsAdditional = addLowTemp && outsideTemperature.toDouble() < lowTemperature /*.toInteger()*/
...
def windowsManagement(desired, bedSensorContactClosed, inside, outsideTemperature, humidity, swing, needCool, inWindowsModes, amplitudeTooHigh){
    ...
    if(controlWindows && windows && !bedSensorContactClosed && !atomicState.override)
    {
        ...
        boolean withinRange = outsideTemperature.toDouble() < outsidetempwindowsH && outsideTemperature.toDouble() > outsidetempwindowsL // stric temp value
        ...
        atomicState.outsideTempAtTimeOfOpening = atomicState.outsideTempAtTimeOfOpening  != null ? atomicState.outsideTempAtTimeOfOpening : outsideTemperature.toDouble() // make sure value is not null
        boolean outsideTempHasDecreased = outsideTemperature.toDouble() < atomicState.outsideTempAtTimeOfOpening - swing // serves mostly to reset opening time stamp
        atomicState.outsideTempAtTimeOfOpening = outsideTempHasDecreased ? outsideTemperature.toDouble() : atomicState.outsideTempAtTimeOfOpening // if outsideTempHasDecreased true, reset outsidetemAtTimeOfOpening stamp so to use outsideTempHasDecreased only once 
...
"""
        ...
        if(inWindowsModes || exception){
            ...
            def time = getWindowsTimeOfOperation(outsideTemperature.toDouble())
                ...
                if(someAreOff || openMore)
                {
                    ...
                    if(atomicState.closedByApp || (openMore && atomicState.openByApp))
                    {
                        ...
                        if(!openMore)
                        {
                            atomicState.lastOpeningTime = now()
                            atomicState.lastOpeningTimeStamp = new Date().format("h:mm:ss a", location.timeZone) // formated time stamp for debug purpose
                            atomicState.outsideTempAtTimeOfOpening = outsideTemperature.toDouble()
...
def getNeed(desired, bedSensorContactClosed, inside){
    ...
    def outsideTemperature = outsideTemp.currentValue("temperature").toDouble() /*.toInteger() */
    def need0 = ""
    def need1 = ""
    def need = []
    def amplThreshold = 2
    def amplitude = Math.abs(inside - desired)
    def swing = outsideTemperature.toDouble() < 10 || outsideTemperature.toDouble() > 24 ? 0.5 : 1 // lower swing when hot or cold outside
    ...
    boolean needCool = !bedSensorContactClosed ? (inWindowsModes ? outsideTemperature.toDouble() >= outsideThres && inside >= coolswing : outsideTemperature.toDouble() >= outsideThres && inside >= desired + swing) : outsideTemperature.toDouble() >= outsideThres + 5 && inside >= desired + swing
...
def getAutoVal(){
    ...
    def humidity = thermostat?.currentValue("humidity") // in auto mode we evaluate based only on inside humidity
...
""" 
    ...
    def maxAH = maxAutoHeat != null ? maxAutoHeat : 28
    def minAC = minAutoCool != null ? minAutoCool : 18
    def minAH = minAutoHeat != null ? minAutoHeat : 18
    def maxAC = maxAutoCool != null ? maxAutoCool : 29
...
def getVariationAmplitude(outside, need){
    ...
    double y // = null // value to find
    double x = outside.toDouble() // current temperature outside
    double ya = desiredVar.toDouble() != null ? desiredVar.toDouble() : 1 // coressponding difference required when outside temperature = xa
    double xa = refTemp.toDouble() != null ? refTemp.toDouble() : 24 // 
    double slope = 0.8
    double m = need == "cool" ? slope : (slope + 0.1)*-1  // slope 
    //def a = -1 // offset
...
def getWindowsTimeOfOperation(outsideTemperature){
    ...
    def x = outsideTemperature.toDouble() // current temperature outside
    def ya = windowsDuration ? windowsDuration : 10 // minimal duration // coressponding duration for when outside temperature = xa
    def xa = outsidetempwindowsL.toDouble() // minimal operation temperature
...
If this message still shows within an hour, check your thermostat configuration..."""
    ...
    def outsideTemperature = outsideTemp?.currentValue("temperature").toDouble()
...
"""
  ...
  if(bedSensorTimeLimit != null) // if user set a time limit
    ...
    if(!atomicState.lastResultWasTrue && result == true)
    {
